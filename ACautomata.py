...
class acmation:
    ...
    # 第一步：模式串建树
    def insert(self, strkey, type, pairno):
...
def readAndBuild(fileDir):
    acp = acmation()
    with open(fileDir, 'r') as reader:
        line = reader.readline()
        i = 1
        while line:
            v, o = line.strip().split('\t')
            acp.insert(v, 'v', i)  # 添加模式串
            acp.insert(o, 'o', i)  # 添加模式串
            line = reader.readline()
            i += 1
        acp.ac_automation()
    return acp
...
def anyPairInSentence(acp, text):
